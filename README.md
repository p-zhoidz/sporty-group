...
На уровне `INFO` логируются стадии одного event и каждой страницы ставок. Стадии отдельной
ставки логируются на уровне `DEBUG`, чтобы событие с миллионами ставок не переполняло
production-лог. Профиль `local` включает `DEBUG` автоматически. Для другого профиля полную
трассировку можно включить так:
...
Каждая запись имеет машиночитаемый формат `[EVENT_NAME][FIELD: value]`. Сквозной поиск
выполняется по `EVENT_ID`, а путь конкретной ставки — по `BET_ID`. Успешные стадии включают
получение HTTP outcome, Kafka broker ack, дедупликацию, чтение и постановку page-task,
создание settlement-command, отправку и получение RocketMQ, а также результат изменения
ставки в БД. Ошибки и сообщения, пропущенные без retry, остаются на `WARN`/`ERROR`.
